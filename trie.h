...
public:
	...
	string Contract(){
		...
		DB* db;
		Options options;
		Status s = DB::Open(options, "Contract.db", &db); // es importante colocar aqui el verdadero nombre de la base de datos y la ubicacion
		assert(s.ok());
		s = db->Get(ReadOptions(), key, &contract);
		delete db;
		SHA256_CTX ctx;
		SHA256_Init(&ctx);
